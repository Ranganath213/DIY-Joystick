...
public class Painter : MonoBehaviour
{
    ...
    public float diffVal = 2f;
    private void OnEnable()
    {
        sprayParticle.Stop();
    }
    public void ColorChanged(Color32 color, SprayChangeEffect effect)
    {
        var transform1 = transform;

        if (effect == SprayChangeEffect.Bring)
        {
            transform1.DORewind();
        ...
        else if (effect == SprayChangeEffect.Shake)
        {
            SetColor(color);
            transform1.DORewind();
            transform1.DOShakeRotation(0.2f, 10f, 4);
        }
    }
    ...
    private void Update()
    {
        ...
        var fingerPos = inputCam.ScreenToWorldPoint(Input.mousePosition);
        ...
        if (!Input.GetMouseButton(0)) return;
            ...
            var diff = ((clickPos - fingerPos) / diffVal ) ;
